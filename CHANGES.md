...
### 问题背景
代理端商品分类菜单中，"混合国家 正常号（二级未知）"等商品在协议号分类中不显示，尽管有价格和库存。

原因分析：
- `AGENT_PROTOCOL_SKIP_KEYWORDS` 默认值为"混合国家,双向号,正常号"
- 这些关键词在协议号检测时被跳过，导致包含这些关键词的商品无法被识别为协议号类商品
- 例如 `leixing = "混合国家 正常号（二级未知）"` 的商品无法匹配任何关键词

### 修复内容
- **修改跳过关键词默认值为空**：所有配置的关键词都会参与匹配
- 这样 "混合国家"、"正常号" 等关键词可以正常匹配，商品会被正确识别为协议号类

---

## 修复：混合国家分类显示问题 (2024-11-25 v3)

### 问题背景
代理端商品分类菜单中，"混合国家 正常号（二级未知）"等商品在协议号分类中不显示，尽管有价格和库存。

原因分析：
- 之前的白名单配置 `AGENT_PROTOCOL_WHITELIST_PATTERNS` 默认值为"混合国家"
- 当商品的 `leixing` 字段以"混合国家"开头时，会被排除出协议号分类
- 导致这些商品既不在协议号分类中显示，也不作为独立分类显示

### 修复内容
- **修改白名单默认值为空**：所有符合协议号特征的商品都会正确归入协议号分类
- 用户可以通过环境变量 `AGENT_PROTOCOL_WHITELIST_PATTERNS` 自定义排除特定分类

---

## 修复：混合国家分类误归入协议号问题 (2024-11-25 v2)

### 问题背景
代理端商品分类菜单显示不全，部分国家/类别（如"混合国家 正常号（二级未知）"、"混合国家 双向号码"等）在总部存在且可通过补货通知 nowuid 直接购买，但在代理机器人的分类菜单中不显示。

原因分析：
- `_is_protocol_like_product()` 函数中的关键词匹配过于宽泛
- 包含"混合国家"、"双向号"、"正常号"的分类被误判为协议号类商品

### 修复内容

#### 1. 新增白名单排除机制
- **配置项**: `AGENT_PROTOCOL_WHITELIST_PATTERNS`（环境变量）
- **默认值**: 空（不排除任何分类）
- **功能**: 以这些模式开头的分类不会被归入协议号，保持原始分类显示

#### 2. 新增跳过关键词配置
- **配置项**: `AGENT_PROTOCOL_SKIP_KEYWORDS`（环境变量）
- **默认值**: `混合国家,双向号,正常号`
- **功能**: 这些关键词在协议号检测时会被跳过，需要更精确的匹配规则

#### 3. 新增二级未知标识符配置
- **配置项**: `AGENT_SECONDARY_UNKNOWN_PATTERNS`（环境变量）
- **默认值**: `(二级未知),（二级未知）`
- **功能**: 用于从分类名称中提取一级分类的回退显示

#### 4. 优化协议号检测逻辑
- **`_is_protocol_like_product()`**: 新增规则0（白名单排除），在所有规则之前优先检查
- **`_is_protocol_like()`**: 同步添加白名单排除逻辑
- **关键词过滤**: 跳过过于泛化的关键词（如单字），避免误判
- **智能跳过**: 使用配置的跳过关键词列表

#### 5. 新增"(二级未知)"回退显示
- **函数**: `_extract_primary_category(leixing)`
- **功能**: 从"混合国家 正常号（二级未知）"提取"混合国家 正常号"作为一级分类
- **应用**: 在 `get_product_categories()` 中，当 leixing 不在 fenlei 中时尝试回退到一级分类

#### 6. 调试模式
- **配置项**: `SHOW_RAW_CATEGORY`（环境变量）
- **默认值**: `0`（关闭）
- **功能**: 设为 `1` 时显示原始分类，便于排查误归并问题

### 配置示例

```bash
# 白名单模式（默认值适合大多数场景）
AGENT_PROTOCOL_WHITELIST_PATTERNS=混合国家 正常号,混合国家 双向号码

# 跳过关键词
AGENT_PROTOCOL_SKIP_KEYWORDS=混合国家,双向号,正常号

# 二级未知标识符
AGENT_SECONDARY_UNKNOWN_PATTERNS=(二级未知),（二级未知）
...
1. 执行 `/diag_sync_stats` 检查分类分布
2. 查看商品分类菜单，确认"混合国家"相关类目出现
3. 通过补货通知 nowuid 直达商品→详情展示与菜单一致

### 影响范围
- 仅影响展示层分类逻辑，不影响购买流程和资金相关功能
- 向后兼容，默认配置即可修复问题
- 所有硬编码值已移至配置，便于维护和自定义
